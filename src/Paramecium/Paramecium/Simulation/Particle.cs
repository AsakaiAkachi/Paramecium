...
namespace Paramecium.Simulation
{
    public class Particle
    {
        ...
        public double PlantDigestiveEfficiency { get; set; }
        public double AnimalPriority { get; set; }
        public double AnimalDigestiveEfficiency { get; set; }
        ...
        //public int CombatCooldown { get; set; }
        ...
        public void OnInitialize()
        {
            ...
            else
            {
                ...
                PlantPriority = Math.Min(Math.Max(1d - (Genes.GeneDiet + 1d) / 2d * 1.5d, 0d), 1d);
                PlantDigestiveEfficiency = Math.Min(Math.Max(1d - (Genes.GeneDiet + 1d) / 2d * 1.5d, 0d), 1d);

                AnimalPriority = Math.Min(Math.Max((Genes.GeneDiet + 1d) / 2d, 0d), 1d);
                //AnimalDigestiveEfficiency = Math.Min(Math.Max(0.333d + (Genes.GeneDiet + 1d) / 3d, 0d), 1d);
                AnimalDigestiveEfficiency = Math.Min(Math.Max(0.5d + (Genes.GeneDiet + 1d) / 4d, 0d), 1d);
            ...
            switch (Type)
            {
                ...
                case ParticleType.Plant:
                    ...
                    if (NextGridBiomassCheck < 0)
                    {
                        ...
                        {
                            // 吸収元グリッドからバイオマスを吸収
                            Biomass += Math.Min(0.15d, TargetGrid.Fertility) * g_Soup.BiomassAmountMultiplier;
                ...
                case ParticleType.Animal:
                    if (Age > 0)
                    {
                        ...
                        else// if (CombatCooldown == 0) // ターゲットが存在していなかった場合
                        {
                                            ...
                                            if (LocalParticlesCount > 0) // 生物に当たった場合
                                            {
                                                ...
                                                for (int k = 0; k < LocalParticlesCount; k++)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        {
                                                            if (Genes.RaceId != TargetParticle.Genes.RaceId && TargetParticle.Age >= 1)
                                                            {
                                            ...
                                            if (Type == ParticleType.Animal) // 自身が動物の場合
                                            {
                                                if (Target.Index == TargetIndex) // 対象がターゲットにしている生物の場合
                                                {
                                                    if (Target.Type == ParticleType.Plant) // ターゲットが植物の場合
                                                    {
                                                        ...
                                                        //double FedBiomassAmount = Math.Min(0.5d + 0.375d * Genes.GeneStrength, Target.Biomass);

                                                        /**
                                                        Target.Biomass -= FedBiomassAmount;
                                                        Biomass += PlantPriority * FedBiomassAmount;
                                                        FedBiomassAmount -= (1d - PlantPriority) * FedBiomassAmount;
                                                        g_Soup.GridMap[(GridPosition.X) + (GridPosition.Y) * local_SizeX].Fertility += FedBiomassAmount;
                                                        ...
                                                        **/
                                                        ...
                                                        Biomass += Math.Min(0.5d + 0.375d * Genes.GeneStrength, Target.Biomass) * PlantDigestiveEfficiency * g_Soup.BiomassAmountMultiplier;
                                                        g_Soup.GridMap[(GridPosition.X) + (GridPosition.Y) * local_SizeX].Fertility += Math.Min(0.5d + 0.375d * Genes.GeneStrength, Target.Biomass) * (1d - PlantDigestiveEfficiency) * g_Soup.BiomassAmountMultiplier;
                                                    ...
                                                    else if (Target.Type == ParticleType.Animal) // ターゲットが動物の場合
                                                    {
                                                        ...
                                                        Random rnd = new Random();
                                                        if (rnd.NextDouble() < 0.125d * Math.Pow(2d, Genes.GeneAgility - Target.Genes.GeneAgility))
                                                        {
                                                            Target.Health -= Math.Max(Genes.GeneStrength * AnimalDigestiveEfficiency * (Genes.GeneStrength - Target.Genes.GeneHardness) * (new Random().NextDouble() * 2d), 0.1d);
                                                        }
                                                        /**
                                                        else if (Target.Genes.GeneStrength > Genes.GeneStrength)
                                                        {
                                                            if (rnd.NextDouble() < 0.125d * Math.Pow(2d, Genes.GeneAgility - Target.Genes.GeneAgility))
                                                            {
                                                                TargetIndex = -1;
                                                                ...
                                                                Target.TargetId = -1;
                                                                Target.Angle += 180d;
                                                                Target.RandomWalkTargetAngle = Target.Angle;
                                                                Target.CombatCooldown = 50;
                                                            }
                                                        }
                                                        **/

                                                        if (rnd.NextDouble() < 0.125d * Math.Pow(2d, Target.Genes.GeneAgility - Genes.GeneAgility))
                                                        {
                                                            Health -= Math.Max(Target.Genes.GeneStrength * Target.AnimalDigestiveEfficiency * (Target.Genes.GeneStrength - Genes.GeneHardness) * (new Random().NextDouble() * 2d), 0.1d);
                                                        }

                                                        if (Target.Health <= 0d)
                                                        {
                                                            /**
                                                            double FedBiomassAmount = Target.Biomass + Target.BiomassStockpiled;
                                                            Biomass += FedBiomassAmount * AnimalPriority;
                                                            FedBiomassAmount -= FedBiomassAmount * (1d - AnimalPriority);
                                                            g_Soup.GridMap[(GridPosition.X) + (GridPosition.Y) * local_SizeX].Fertility += FedBiomassAmount;
                                                            **/

                                                            Biomass += (Target.Biomass + Target.BiomassStockpiled) * AnimalDigestiveEfficiency * g_Soup.BiomassAmountMultiplier;
                                                            g_Soup.GridMap[(GridPosition.X) + (GridPosition.Y) * local_SizeX].Fertility += (Target.Biomass + Target.BiomassStockpiled) * (1d - AnimalDigestiveEfficiency) * g_Soup.BiomassAmountMultiplier;
                                                            Target.Biomass = 0;
                                                            Target.Health = 0;
                                                        }
                                                    ...
                                                    if (Target.Biomass <= 0 || (Target.Type == ParticleType.Animal && Target.Health <= 0))
                                                    {
        ...
        public void LateUpdate(int threadId)
        {
            ...
            if (Type == ParticleType.Animal)
            {
                if (Age > 0)
                {
                    ...
                    //double DecreaseBiomassAmount = Math.Min(0.05d + (0.15d * Vector2D.Size(Velocity)), Biomass);
                    //Biomass -= DecreaseBiomassAmount;
                    //g_Soup.GridMap[(GridPosition.X) + (GridPosition.Y) * local_SizeX].Fertility += DecreaseBiomassAmount;

                    g_Soup.GridMap[(GridPosition.X) + (GridPosition.Y) * local_SizeX].Fertility += Math.Min(0.05d + (0.15d * Vector2D.Size(Velocity)), Biomass) * g_Soup.BiomassAmountMultiplier;
            ...
            if (Velocity != Vector2D.Zero || g_Soup.ElapsedTimeStep == 0)
            {
                ...
                if (Vector2D.Size(Velocity) < 0.0001d)
                {
        ...
        public void OnStepFinish(int threadId)
        {
            ...
            if (!IsAlive)
            {
                g_Soup.GridMap[GridPosition.X + GridPosition.Y * local_SizeX].Fertility += (Biomass + BiomassStockpiled) * g_Soup.BiomassAmountMultiplier;
                g_Soup.GridMap[GridPosition.X + GridPosition.Y * local_SizeX].LocalParticles.Remove(Index);
                g_Soup.GridMap[GridPosition.X + GridPosition.Y * local_SizeX].LocalParticleCount--;
                if (Type == ParticleType.Plant) g_Soup.GridMap[GridPosition.X + GridPosition.Y * local_SizeX].LocalPlantCount--;
                else g_Soup.GridMap[GridPosition.X + GridPosition.Y * local_SizeX].LocalAnimalCount--;
